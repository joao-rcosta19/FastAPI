...
async def create_item(item: Item):
    ...
    if validar == False:
        ...
        elif frase not in item.email:
            a = buscar(item.email, "lista_email2", "email")
        else:
            teste = item.email.replace(" ","")
            a = buscar(teste, "lista_email2", "email")
            item.email = teste
        #inserindo e validando email
        if a != True:
            inserindoEmail(item.email, "lista_email2", item.chave)
        vEmail = validaEmail(item.email) #acrecentar codigo de validação
        if vEmail == True: 
